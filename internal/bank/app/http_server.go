...
func (a *app) errorHandler(err error, c echo.Context) {
	...
	if !c.Response().Committed {
		if c.Request().Method == http.MethodHead {
			...
			err = http_errors.NewNotFoundError(c, msg, a.cfg.Http.DebugErrorsResponse)
		} else if code == http.StatusMethodNotAllowed {
			// if method not allowed, return 405
			err = http_errors.NewMethodNotAllowedError(c, msg, a.cfg.Http.DebugErrorsResponse)
		} else {
			err = http_errors.NewInternalServerError(c, msg, a.cfg.Http.DebugErrorsResponse)
		}
		if err != nil {
